...
export const themLichKham = ({ doctorId, clinicId, timeSlot, startTime, endTime, price, appointmentDate, status }) => new Promise(async (resolve, reject) => {
    try {
        const newSchedule = await db.Schedule.findOrCreate({
            where: { doctorId, clinicId, status: 'available', },
            defaults: {
                doctorId,
                clinicId,
                timeSlot,
...
export const datLich = ({ id_lichkham }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: schedule ? '1' : '0',
            mess: check ? "Lịch khám đã được hủy hoặc đã hoàn thành" : "Đặt Lịch Thành Công"
        });
    } catch (e) {
        reject(e)
    }
